...
public class ConsentValidator {
    ...
    public static void validateCreateConsent(ConsentRequest consentRequestBody) {
        nullValidation(consentRequestBody);
        validateCommonParameters(consentRequestBody);
        validateConsentDetail(consentRequestBody.getTxnid(), consentRequestBody.getConsentDetail());
    }

    public static void validateStatus(String txnId, String status) {
        ArgsValidator.validateStatus(txnId, status);
    }

    protected static void nullValidation(ConsentRequest consent) {
    ...
    protected static void nullValidateConsentDetail(String txnId, ConsentDetail consentDetail) {
    ...
    protected static void validateCommonParameters(ConsentRequest consent) {
    ...
    protected static void validateConsentDetail(String txnId, ConsentDetail consentDetail) {
    ...
    public static void validateConsentMode(String txnId, String consentMode) {
        if (ConsentMode.get(consentMode) == null)
            throw InvalidRequest.with(txnId, "Invalid consent mode");
    }

    public static void validateFetchType(String txnId, String fetchType) {
        if (FetchType.get(fetchType) == null)
            throw InvalidRequest.with(txnId, "Invalid Fetch type");
    }

    public static void validateConsentTypes(String txnId, List<String> consentTypes) {
        if (consentTypes.stream().anyMatch(e -> ConsentType.get(e) == null))
            throw InvalidRequest.with(txnId, "Invalid consent types");
    }

    public static void validateFITypes(String txnId, List<String> fiTypes) {
    ...
    protected static void notNegative(String txnId, int integer, String parameter) {
        if (integer < 0)
